...
class CommandInterpreter(object):
    ...
    def ADD(self, command, i, carry=False, inPlace=False):
        ...
        if argumentType == self._ERROR: return self.ERRSTR

        y = self.getValue(command[2], argumentType)

        if carry: y += self.machine.getFlag('C')
    ...
    def XCHG(self, command, i):
        ...
        if argType1 == self._ERROR: return self.ERRSTR
        argType2 = self.testArgument(command, 2, i, [self._REG])
        if argType2 == self._ERROR: return self.ERRSTR
