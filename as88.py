...
class CommandInterpreter:
        ...
        if command[1] == "SP" and command[2].isdigit():  # TODO: REPLACE THIS
            for j in range(int(command[2]) / 2):
                if len(self.assembler.stackData) > 0:
                    ...
                    self.assembler.updateStack()
        if command[1].isdigit():
            self.assembler.outPut("Error on line " + str(i) + ". Add cannot have a numerical first argument.")
            self.assembler.stopRunning(-1)
            return
    ...
    def cmpb(self, command, i):
        ...
        for x in a:
            if x in ['AH', 'AL', 'BH', 'BL', 'CH', 'CL', 'DH', 'DL']:
                ...
                self.assembler.outPut("Illegal argument for cmpb on line %d. %s is a 16 bit register, perhaps you meant one of the 8 bit %s or %s registers?" % (i, x, x[0] + "H", x[0] + "L"))
                self.assembler.stopRunning(-1)
                return
            elif type(x) == type(""):
                ...
                if len(x) != 1:
                    self.assembler.outPut("Illegal argument %s for cmpb on line %d. cmpb expects all strings to be ONE byte in length." % (x, i))
                    self.assembler.stopRunning(-1)
                    return
                else:
                    try:
    ...
    def incdec(self, command, i, p):
        ...
        if command[1] in self.getRegisters().keys():
            ...
            self.assembler.outPut("Invalid " + command[0] + " on line " + str(i) + ". " + command[0] + " expects its argument to be either a register or memory address.")
    ...
    def jmp(self, command, i, referer="jmp"):
        ...
        if command[1] in self.assembler.lookupTable.keys():
            if type(self.assembler.lookupTable[command[1]]) == self.assembler.LIST_TYPE:
            ...
            self.assembler.outPut("Error on line " + str(i) + ". The label " + command[1] + " is not defined for " + referer + "-ing to.")
    ...
    def mov(self, command, i):
        ...
        if command[1] in self.assembler.registers.keys():
            ...
            self.assembler.outPut("Error, you made your first argument a hex digit. MOV expects its first argument to be a register, or a memory address.")
        else:
            self.assembler.outPut("MOV expects its first argument to be a memory address or register, and second to be a memory address, register, immediate value, or local variable.")
    ...
    def pop(self, command, i):
        ...
        if command[1] in self.getRegisters():
    ...
    def push(self, command, i):
        ...
        if command[1].strip("abcdef").isdigit():  # pushing a number to the stack, it's prolly hex so ignore the A-F chars
            ...
            self.assembler.updateStack()
        elif command[1] in self.getRegisters():
            ...
            if temp in self.assembler.BSS.keys():
                ...
                self.assembler.outPut("Error on line " + str(i) + ". I don't understand what (" + temp + ") is")
                self.assembler.stopRunning(-1)
        else:
            print(command)
            self.assembler.outPut("Unknown error on line " + str(i) + ".")
            self.assembler.stopRunning(-1)
    ...
    def sys(self, command, i):
        ...
        if len(self.assembler.stackData) == 0:
            self.assembler.outPut("Invalid system trap: SYS called on line %d without any arguments on stack." % i)
            self.assembler.stopRunning(-1)
        elif not int(self.assembler.stackData[-1]) in self.sysCodes.values():
            self.assembler.outPut("Invalid system trap on line %d: The first argument \"%s\" on the stack is not understood" % (i, self.assembler.stackData[-1]))
            self.assembler.stopRunning(-1)
        else:
            if int(self.assembler.stackData[-1]) == self.sysCodes["_EXIT"]:
                self.assembler.stopRunning()
            elif int(self.assembler.stackData[-1]) == self.sysCodes["_GETCHAR"]:
                self.assembler.getChar()
                ...
                try:
                    ...
                    self.assembler.outPut(formatStr % tuple(args))
                except IndexError:
                    self.assembler.stopRunning(-1)
                    self.assembler.outPut("Invalid system trap on line %d. Invalid number of arguments with _PRINTF." % i)
    ...
    def xchg(self, command, i):
        ...
        if command[1] in self.assembler.registers and command[2] in self.assembler.registers:
            ...
            self.assembler.outPut("Error on line " + str(i) + ". XCHG expects both its arguments to be registers.")
